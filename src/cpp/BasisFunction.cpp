...
double BasisFunction::d(const std::array<double, 3>& point) const {
    ...
    if (index_ == "z2") {
        for (size_t i = 0; i < coefficients_.size(); ++i)
            res += coefficients_[i] / std::sqrt(6.0) * (dz*dz - 0.5*dx*dx - 0.5*dy*dy) * std::exp(-exponents_[i] * squared_radius_vector);
        ...
        for (size_t i = 0; i < coefficients_.size(); ++i)
            res += coefficients_[i] / std::sqrt(2.0) * (dx*dx - dy*dy) * std::exp(-exponents_[i] * squared_radius_vector);
